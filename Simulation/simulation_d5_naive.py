...
GEs_mean = [np.mean(trails_d5_naive['GEs'])] * 70
AEs_mean = np.mean(trails_d5_naive['AEs'], axis=1)
LEs_mean = np.mean(trails_d5_naive['LEs'], axis=1)
AE_adapts_mean = np.mean(trails_d5_naive['AE_adapts'], axis=1)
AE_logs_mean = np.mean(trails_d5_naive['AE_logs'], axis=1)
AE_log_actives_mean = np.mean(trails_d5_naive['AE_log_actives'], axis=1)
...
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/naive_d5_log.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
...
ax.set_xlabel('Blocks number \n(d=5, same block size)', fontsize='13')
ax.set_ylabel('MSE (Naive)', fontsize='13')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/naive_d5_active.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
