...
# error_validation = GE_naivek(x_train, y_train, x_test, y_test, h, d)
#error_validation = GE_gaussiank(x_train, y_train, x_test, y_test, h, d, s)
#error_validation = GE_Epank(x_train, y_train, x_test, y_test, h, d)   # replaced to the specific function
    ...
    if p2 <= len(b_y):
        ...
        for i in range(1, 6):
            ...
            if cv_p1 <= cv_p2:
                ...
                k_opt1 = p2
                # print('optimal k: %s, tuning at: %s'% (p2, p2-p1))
                p1, p2 = math.ceil(p2 - 0.5 * p1), math.ceil(p2 + 0.5 * p1)  # p1每次都是变化的，所以精度虽然能达到2，但是tuning并不是从大到小地变化
...
def global_parameter_k_d5(x_tra, y_tra, d):
    ...
    for i in range(6):
        ...
        if cv_p1 <= cv_p2:
            print('optimal k: %s, tuning at: %s' % (p1, p2 - p1))
            cv_compare, k_opt1 = cv_p1, p1
            p1, p2 = p1 - int(15 / (2 ** i)), p1 + int(15 / (2 ** i))
        else:
            cv_compare, k_opt1 = cv_p2, p2
            print('optimal k: %s, tuning at: %s' % (p2, p2 - p1))
            p1, p2 = p2 - int(15 / (2 ** i)), p2 + int(15 / (2 ** i))
    ...
    if p2 <= len(b_y):
        ...
        for i in range(6):
            ...
            if cv_p1 <= cv_p2:
                ...
                cv_compare = cv_p1
                k_opt1 = p1
                p1, p2 = p1 - int(start_p / (2 ** i)), p1 + int(start_p / (2 ** i))
            else:
                # print('p1>p2 ---- ')
                cv_compare = cv_p2
                k_opt1 = p2
                #print('optimal k: %s, tuning at: %s'% (p2, p2-p1))
                p1, p2 = p2 - int(start_p / (2 ** i)), p2 + int(start_p / (2 ** i))
...
'''2.2 (3) opt_k for each block in a certain fen_num / division, remember to replace: d=1, d=5 !!'''
def local_parameter_k_d(x_tra, y_tra, m_knn, p1, p2, d):
    k_adapt_blocks = []
    for j in range(m_knn):
        k0 = opt_local_k_d5(x_tra, y_tra, m_knn, p1, p2, d, j)           # replace: d = 1 or d = 5 !!
...
def GE_Epank(x_tra, y_tra, x_tes, y_tes, h, d):
    ...
    a1 = x_tes - x_tra
    b1 = h
    dis = np.divide(a1, b1, out=np.zeros_like(a1, dtype=np.float64), where=b1 != 0)  # 0/0 = 0
...
def AE_active_epank(x_tra, y_tra, x_tes1, y_tes, m, h_adapt, d):
    ...
    for j in range(m):
        ...
        a2 = x_tes - b_x
        b2 = h
        dis = np.divide(a2, b2, out=np.zeros_like(a2, dtype=np.float64), where=b2 != 0)  # 0/0 = 0
        dis_norm = (np.linalg.norm(dis, axis=1)) ** 2
        weight = np.where((1 - dis_norm) > 0, 1 - dis_norm, 0)
        a0 = np.dot(weight, b_y)
        b0 = np.sum(weight, axis=1)
...
(1) "assign = random.randint(1, 4)":  to prove the autonomy on algorithm. assign=1, gaussian; assign=2,epan; assign=3,naive; assign=4,knn
...
def hybrid_algorithm_test(x_tra, y_tra, x_tes, y_tes, adapt_gau, adapt_epan, adapt_nai, adapt_knn, d, gap, fen_num):
    Hybrid_Error= []
    for i in range(fen_num):    # change to range(fen_num-1, fen_num) when calculating on perturbed data
...
# print('y(dose)=21mg所对应的index是：%s, 所对应的归一化后的y_scaled是：%s' % (y.index(21), y_scaled[y.index(21)]))  # 24, 0.08293111 （0.08293111013602456）
# print('y(dose)=49mg所对应的index是：%s, 所对应的归一化后的y_scaled是：%s' % (y.index(49), y_scaled[y.index(49)]))  # 0, 0.20579201 （0.20579201404124614）
def clinical_percentage(y_pre, y_tes):
    num_low, num_high, num_intermediate = 0, 0, 0
    low_under, low_over, low_ideal = 0, 0, 0
    high_under, high_over, high_ideal = 0, 0, 0
    intermediate_under, intermediate_over, intermediate_ideal = 0, 0, 0
    dose_21 = 0.08293111013602456   # after preprocessing.MinMaxScaler()
    dose_49 = 0.20579201404124614

    for i in range(len(y_tes)):
        if y_tes[i] <= dose_21:
            num_low += 1
            if y_pre[i] < 0.8 * y_tes[i]: low_under += 1
            if y_pre[i] > 1.2 * y_tes[i]: low_over += 1
            if y_pre[i] <= 1.2 * y_tes[i] and y_pre[i] >= 0.8 * y_tes[i]: low_ideal += 1

        if y_tes[i] >= dose_49:
            num_high += 1
            if y_pre[i] < 0.8 * y_tes[i]: high_under += 1
            if y_pre[i] > 1.2 * y_tes[i]: high_over += 1
            if y_pre[i] <= 1.2 * y_tes[i] and y_pre[i] >= 0.8 * y_tes[i]: high_ideal += 1

        if y_tes[i] < dose_49 and y_tes[i] > dose_21:
            num_intermediate += 1
            if y_pre[i] < 0.8 * y_tes[i]: intermediate_under += 1
            if y_pre[i] > 1.2 * y_tes[i]: intermediate_over += 1
            if y_pre[i] <= 1.2 * y_tes[i] and y_pre[i] >= 0.8 * y_tes[i]: intermediate_ideal += 1

    # percentahe on low dose group
    perc_low_under = np.divide(low_under, num_low, out=np.zeros_like(low_under, dtype=np.float64), where=num_low != 0)  # 0/0 = 0
    perc_low_over = np.divide(low_over, num_low, out=np.zeros_like(low_over, dtype=np.float64), where=num_low != 0)
    perc_low_ideal = np.divide(low_ideal, num_low, out=np.zeros_like(low_ideal, dtype=np.float64), where=num_low != 0)
    # percentahe on high dose group
    perc_high_under = np.divide(high_under, num_high, out=np.zeros_like(high_under, dtype=np.float64), where=num_high != 0)
    perc_high_over = np.divide(high_over, num_high, out=np.zeros_like(high_over, dtype=np.float64), where=num_high != 0)
    perc_high_ideal = np.divide(high_ideal, num_high, out=np.zeros_like(high_ideal, dtype=np.float64), where=num_high != 0)
    # percentahe on intermediate dose group
    perc_intermediate_under = np.divide(intermediate_under, num_intermediate, out=np.zeros_like(intermediate_under, dtype=np.float64), where=num_intermediate != 0)
    perc_intermediate_over = np.divide(intermediate_over, num_intermediate, out=np.zeros_like(intermediate_over, dtype=np.float64), where=num_intermediate != 0)
    perc_intermediate_ideal = np.divide(intermediate_ideal, num_intermediate, out=np.zeros_like(intermediate_ideal, dtype=np.float64), where=num_intermediate != 0)

    return perc_low_under, perc_low_over, perc_low_ideal, \
           perc_high_under, perc_high_over, perc_high_ideal, \
           perc_intermediate_under, perc_intermediate_over, perc_intermediate_ideal


# s1 = np.array([30,18,61,61,10,10])
# s2 = np.array([21,20,50,50,30,30])
# a = clinical_percentage(s1, s2)
# print(a)


def clinical_num(y_tes):
    num_low, num_high, num_intermediate = 0, 0, 0
    dose_21 = 0.08293111013602456   # after preprocessing.MinMaxScaler()
    dose_49 = 0.20579201404124614
    for i in range(len(y_tes)):
        if y_tes[i] <= dose_21: num_low += 1
        if y_tes[i] >= dose_49: num_high += 1
        if y_tes[i] < dose_49 and y_tes[i] > dose_21: num_intermediate += 1

    return num_low, num_high, num_intermediate
